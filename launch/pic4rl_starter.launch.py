...
def print_params(context, *args, **kwargs):
    ...
    if not (
        ...
        configured_params = main_params.perform(context=context)
    ...
    task_node = Node(
        package=pkg_name.perform(context=context),
        ...
        parameters=[
            main_params_dict,
            {"package_name": pkg_name.perform(context=context)},
            {"main_params_path": main_params.perform(context=context)},
            {"training_params_path": training_params.perform(context=context)},
        ],
    )
    return [task_node]
